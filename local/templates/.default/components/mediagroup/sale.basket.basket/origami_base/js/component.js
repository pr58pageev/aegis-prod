  ...
  BX.Sale.BasketComponent = {
    ...
    init: function init(parameters) {
      ...
      if (this.useItemsFilter) {
        ...
        if($('.active_custom_click').html() != undefined){
          this.params.EXCLUDE_DELAY = 'N';
    ...
    deleteBasketItems: function deleteBasketItems(items, restore, _final) {
      ...
      for (var i in items) {
        if (items.hasOwnProperty(i)) {
          this.deleteBasketItem(items[i], restore, _final);
        }
      }
    },
    deleteBasketItem: function deleteBasketItem(itemId, restore, _final2) {
