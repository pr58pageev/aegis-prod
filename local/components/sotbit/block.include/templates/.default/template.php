...
<div class="blocks-wrapper">
            ...
            foreach ($arResult['AVAILABLE_BLOCKS'] as $section => $blocks) {
              foreach ($blocks as $block) {
                ...
                <div class="landing-ui-card1 landing-ui-card-block-preview
							<?= ($i == 0) ? 'landing-ui-card-block-active' : 'landing-ui-card-block-noactive' ?>"
                     data-code="<?= $block['CODE'] ?>" data-section="<?= $section ?>">
                  ...
                  <div class="landing-ui-card-body">
                    <div class="landing-ui-card-block-preview-image-container">
                      <img style="opacity: 1" src="<?= SITE_TEMPLATE_PATH ?>/assets/img/loader_lazy.svg"
                           data-src="<?= $block['PREVIEW'] ?>" alt="<?= $block['SETTINGS']['block']['name'] ?>"
                           title="<?= $block['SETTINGS']['block']['name'] ?>" class="lazy-ui">
  ...
  <div
      id="blocks_<?= $arParams['PART'] ?>"<? if (!isset($_COOKIE["origamiConstructMode"]) || $_COOKIE["origamiConstructMode"] != "Y"): ?> class="site-builder__hide"<? else: ?>class="site-builder__show"<? endif; ?>>
