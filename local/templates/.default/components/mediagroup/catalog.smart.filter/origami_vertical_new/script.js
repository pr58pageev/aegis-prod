...
JCSmartFilter.prototype.reload = function (input) {
    ...
    if (this.form) {
        ...
        for (prop in values) {
            ...
            values['arrFilter_494_MIN'] = $('#arrFilter_494_MIN').val();
        values['arrFilter_494_MAX'] = $('#arrFilter_494_MAX').val();
        ...
        function setDropdownSize() {
            if (document.getElementById(id)) {
                ...
                if (mql.matches) {
                    ...
                    thisDropDown.style.minWidth = "210px";
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    hideShowAllRadiosBtns();
});

window.addEventListener('load', hideShowAllRadiosBtns);

function hideShowAllRadiosBtns() {
    let radioWrappers = document.querySelectorAll('.radio_container_wrapper');

    for (let i = 0; i < radioWrappers.length; i++) {
        if (radioWrappers[i].querySelector('.radio_container_wrapper__resizer').clientHeight < 113) {
            radioWrappers[i].querySelector('.bx_filter_parameters_open_all').style.display = 'none';
        } else {
            radioWrappers[i].querySelector('.radio_container_wrapper__resizer').style.height = 113 + 'px';
        }
    }
}

function openAllToggleRadio(buttonsWrapper) {
    let wrapper = buttonsWrapper.closest('.radio_container_wrapper');

    if (buttonsWrapper.dataset.open === 'true') {
        delete buttonsWrapper.dataset.open;
        buttonsWrapper.classList.remove('open');

        wrapper
            .querySelector('.radio_container_wrapper__resizer')
            .style
            .height = 113 + 'px';
    } else {
        buttonsWrapper.dataset.open = 'true';
        ...
        wrapper
            ...
            .height =
            wrapper
                .querySelector('.radio_container_wrapper__resizer > div').clientHeight + 'px';
    }
}
...
$(document).ready(function () {
    ...
    let filters = document.querySelector('.bx_filter');
    let filterBlock = filters.querySelectorAll('.bx_filter_parameters_box');
    let filterBtn = filters.querySelector('.bx_filter_parameters_open_all');
...
function setActiveState (element) {
    let checkedItems = [];
    let itemsFilter = $(element).siblings(".blank_ul_wrapper").find(".hidden_filter_props").find(".bx_filter_parameters_box_checkbox ");
    let blockFilter = $(element).siblings(".blank_ul_wrapper");
