...
{
    $arParams['BRAND_PROP_CODE'] = 'MARKA';
    $iblockBrand = BRAND_IBLOCK;
    if(!empty($arParams['CUSTOM_BRAND_CODE'])){
        $arParams['BRAND_PROP_CODE'] = $arParams['CUSTOM_BRAND_CODE'];
        $iblockBrand = $arParams['IBLOCK_ID'];
    }
     $arSelect = Array("*",'PROPERTY_HIDDEN');
    $arFilter = Array("IBLOCK_ID" => $iblockBrand, "CODE" => $arResult['VARIABLES']['ELEMENT_CODE']);
...
$XML_ID = $ob['XML_ID'];
    ...
    $UserField = CIBlockPropertyEnum::GetList(array(), array("XML_ID" => $XML_ID));
   if($UserFieldAr = $UserField->GetNext())
   {
      $value_name = $UserFieldAr['VALUE'];
   }

 

    
//'PROPERTY_'.$arParams['BRAND_PROP_CODE'].'.CODE' => $arResult['VARIABLES']['ELEMENT_CODE'],
 
    $arFilter = array("IBLOCK_ID" => $iblockProd,
        "ACTIVE" => "Y",
        'PROPERTY_'.$arParams['BRAND_PROP_CODE'].'_VALUE'=>$value_name,
       '!PROPERTY_'.$arParams['BRAND_PROP_CODE']=>false,
        'PROPERTY_'.VIDIMOST_OFFER_PROP_CODE => 1
    );
    $arSelect = array("ID", "IBLOCK_SECTION_ID",'PROPERTY_'.$arParams['BRAND_PROP_CODE']);
		...
		<div class="block_main_left_menu__content active no-padding">
			...
			<div class="panel_filter_sort">
                ...
                if(Loader::includeModule('sotbit.origami')){
                    $sort = array();
                    $sort = $APPLICATION->IncludeFile( SITE_DIR . "include/sotbit_origami/sort.php", Array(), Array());
                }
...
//'SHOW_MAX_QUANTITY'         => Config::get('SHOW_STOCK_MODE'),
			...
			<div class="line"></div>
			<a
					href="<?= $arResult["FOLDER"].$arResult["URL_TEMPLATES"]["news"] ?>"
