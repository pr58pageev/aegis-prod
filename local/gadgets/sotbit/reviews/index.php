...
if(Loader::includeModule('sotbit.cabinet') && Loader::includeModule('sotbit.reviews') && $idUser > 0)
{
	$limit = 1;
	...
	$listReviews = new \Sotbit\Cabinet\Personal\Reviews();
		...
		<div class="personal-container-reviews">
			...
			<div class="personal-container-reviews-status">
				...
				<span class="<?= ($review->isModerated() ? 'reviews-status-published' : 'reviews-status-unpublished') ?>">
					<?= ($review->isModerated() ? Loc::getMessage('SOTBIT_CABINET_REVIEWS_MODERATED') :
						Loc::getMessage('SOTBIT_CABINET_REVIEWS_NOT_MODERATED')) ?>
				</span>
			</div>
	...
	if($arParams['G_REVIEWS_PATH_TO_REVIEWS'])
	{
		...
		<div class="gdhtmlareachlink">
			<a href="<?=$arParams['G_REVIEWS_PATH_TO_REVIEWS']?>">
				<?=Loc::getMessage('SOTBIT_CABINET_REVIEWS_ALL')?>
			</a>
		</div>
		<?
	}
