...
<script>
    ...
    function start(file)
    {
        ...
        if(!sessid){
            get_sessid_and_file(file);
        }else {
            query_1c(file);
        }
    }

    function get_sessid_and_file(file){
        var import_1c=createHttpRequest();
        r = "/bitrix/admin/1c_exchange.php?type=catalog&mode=checkauth";
        import_1c.open("GET", r, true);

        import_1c.onreadystatechange = function() {
            a=log.innerHTML;
            if (import_1c.readyState == 4 && import_1c.status == 200) {
                sessid = import_1c.responseText.substr(58,32);
                log.innerHTML=a+"Получили сессию: "+sessid+'<br>';
                query_1c(file);
            }
        };
    ...
    function query_1c(file)
    {
        ...
        if (zup_import==true)
        {
            r="/bitrix/admin/1c_intranet.php?type=catalog&mode=import&sessid="+sessid+"&filename="+file;
        } else{
            r="/bitrix/admin/1c_exchange.php?type=catalog&mode=import&sessid="+sessid+"&filename="+file;
        }
