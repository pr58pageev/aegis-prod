...
if(Loader::includeModule('sotbit.b2bcabinet') && Loader::includeModule('sale') && $idUser > 0) {
    ...
    foreach($orders as $order) {
       if (($arOrder = CSaleOrder::GetByID($order->getId())))
        {
          
          $acc = $arOrder['ACCOUNT_NUMBER'];
        }
        ...
        <div class="widget_content widget_links orders">
            <div class="widget_order_content">
                ...
                <div class="widget_order_header">
                    <a href="<?=$order->getUrl($arParams['G_ORDERS_PATH_TO_ORDER_DETAIL'])?>" title="<?=Loc::getMessage('GD_SOTBIT_CABINET_ORDER_ORDER')?> <?=$acc?>">
                        <h6><?=Loc::getMessage('GD_SOTBIT_CABINET_ORDER_ORDER')?> <?=$acc?></h6> <i class="icon-arrow-right13 mr-2"></i>
                    </a>
                </div>
