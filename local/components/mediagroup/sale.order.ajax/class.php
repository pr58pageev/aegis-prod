...
class SaleOrderAjax extends \CBitrixComponent
{
    ...
    public function onPrepareComponentParams($arParams)
    {
        ...
        if (empty($_GET['ID_ORDER'])) {
            $_GET['ID_ORDER'] = $arParams['ID_ORDER'];
        }
    ...
    protected function initProperties(Order $order, $isPersonTypeChanged)
    {
       ...
       /* //365mg
        if (empty($this->arUserResult['ORDER_PROP'][37]) && empty($_POST['ORDER_PROP_37'])):
            unset($this->arUserResult['ORDER_PROP'][18]);
        endif;*/
    ...
    protected function calculateDeliveries(Order $order)
    {
        ...
        if (!empty($this->arDeliveryServiceAll)) {
            ...
            foreach ($this->arDeliveryServiceAll as $deliveryId => $deliveryObj) {
                ...
                $delivId = (int)$shipment->getDeliveryId();

                if (!empty($_POST['DELIVERY_ID'])) {
                    $delivId = (int)$_POST['DELIVERY_ID'];
                }

                if ($delivId === $deliveryId) {
    ...
    protected function saveOrderAjaxAction()
    {
        ...
        if ($this->checkSession) {
            ...
            if (empty($this->arResult["ERROR"])) {
                ...
                $arOrderRes['REDIRECT_URL'] = '/b2bcabinet/order/detail/' . $this->arResult["ACCOUNT_NUMBER"] . '/';
    ...
    public function initShipment(Order $order)
    {
        ...
        //$shipment = $order->getShipmentCollection()->getNotSystemItems();
...
//            }
//        }
//        pre($shipmentItemCollection);

//        $basket = \Bitrix\Sale\Basket::create($this->getSiteId());

        /** @var Sale\BasketItem $item */
        foreach ($order->getBasket() as $item) {

//            $newitem = $basket->createItem('catalog', $item->getProductId());
...
//            foreach ($basketPropertyCollection->getPropertyValues() as $basketProperty){
//
//                $newitem->setField($basketProperty['CODE'],$basketProperty['VALUE']);
//
//                // любое поле свойства 'NAME','CODE','VALUE','SORT','XML_ID'
//            }

//            pre($item->getId());
//            pre($item->getProductId());
//            pre($newitem->getQuantity());
//            pre($item->getField('ORDER_ID'));
////            pre($item->setField('ORDER_ID',false));
//            pre($item);
...
//        pre($basket);
    ...
    protected function createOrder($userId)
    {
        ...
        //$order = Sale\Order::load($_GET['ID_ORDER']); //by vitaly
    ...
    protected function getOrder($userId)
    {
        ...
        $order = $orderClassName::load($_GET['ID_ORDER']);
...
//        $order->save();
    ...
    protected function processOrderAction()
    {
        ...
        $this->order = $this->createOrder($userId); //Sale\Order::load($_GET['ID_ORDER']);
        $this->prepareResultArray();
        ...
        if ($this->isOrderConfirmed && $isActiveUser && empty($arResult["ERROR"])) {
            ...
            if (empty($arResult["ERROR"])) {
                ...
                //$arResult["REDIRECT_URL"] = $APPLICATION->GetCurPage().'?ID_ORDER='.$_GET["ID_ORDER"];
                $arResult['REDIRECT_URL'] = '/b2bcabinet/order/detail/' . $arResult["ACCOUNT_NUMBER"] . '/';
