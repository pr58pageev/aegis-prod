...
<? if (!empty($arResult['ITEMS']['SECTION_ID']['NAME']) && !empty($arResult['ITEMS']['SECTION_ID']['SORT_VALUES'])):
?>
  <style>
    .perfectscroll {
    ...
    .bx_filter_parameters_open_all_open {
      border-bottom: 1px solid #244ed5;
      color: #244ed5;
    }
...
<form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>"
  ...
  <div class="card index_blank-categories card-collapsed">

    <div class="card-header bg-transparent header-elements-inline">
      <span
          class="text-uppercase font-size-sm font-weight-semibold"><?= $arResult['ITEMS']['SECTION_ID']['NAME'] ?></span>
    ...
    <? if (is_array($arResult['ITEMS']['SECTION_ID']['SORT_VALUES'])): ?>
      <div class="card-body border-0 p-0" style="display: none;">
        <ul class="nav nav-sidebar mb-2" style="padding-bottom: 80px;">
          <?

          foreach ($arResult['ITEMS']['SECTION_ID']['SORT_VALUES'] as $VALUE): ?>
            <? if (!empty($VALUE['VALUE'])): ?>
              <li class="nav-item <?= (!empty($VALUE['CHILDS']) ? 'nav-item-submenu' : '') ?> catalog_section">
                <a class="nav-link">
                  <div class="form-check">
                    <label class="form-check-label bx_filter_parameters_box_checkbox">
                      <input type="checkbox" class="form-input-styled checkbox__input"
                             data-fouc id="<?= $VALUE["CONTROL_ID"] ?>"
                             value="<?= $VALUE["HTML_VALUE"] ?>"
                             name="<?= $VALUE["CONTROL_NAME"] ?>"
                        <?= $VALUE["CHECKED"] || $VALUE['CHILD_SELECTED'] == 'Y' ? 'checked="checked"' : '' ?>
        ...
        <div class="bx_filter_block button" style="position: absolute;
    z-index: 99999;
    width: 100%;
    top: 0;
    right: -100%;">
    ...
    margin-bottom: 10px;" class="bx_filter_search_reset fonts__main_comment btn btn-light" type="submit"
                   id="del_filter1"
  ...
  <div class="index_blank-filter" <? if (empty($_GET['set_filter'])): ?>style="display: none;"<? endif; ?>>
    <div class="card bx_filter_section bx_filter card-collapsed">
      <div class="card-header bg-transparent header-elements-inline">
      ...
      <div class="card-body" style="display: none;">
        ...
        foreach ($arResult["ITEMS"] as $key => $arItem) {
          ...
          if (isset($arItem["PRICE"])):
            ...
            <div class="bx_filter_parameters_box active form-group"
              ...
              <div
                  class="bx_filter_parameters_box_title fonts__middle_text font-size-xs text-uppercase text-muted mb-3">
                                <span>
        ...
        foreach ($arResult["ITEMS"] as $key => $arItem) {
          ...
          foreach ($arItem['VALUES'] as $key => $value) {
            if ($value['DISABLED'] != '1' && !empty($value)) {
              $show = true;
            }
          }
            ...
            <div class="bx_filter_block bx_filter_block_wrapper" data-role="bx_filter_block">
              <div class="bx_filter_parameters_box_container">
                ...
                ?>
                  ...
                  <div
                      class="blank_ul_wrapper type-checkbox <?= ($totalValues > $maxDisplay ? "perfectscroll" : "") ?>">
                    <?
                    ...
                    foreach ($arItem["VALUES"] as $val => $ar):
                      ...
                      <div
                          class="bx_filter_parameters_box_checkbox form-check <?= $class ?> <? echo $ar["DISABLED"] ? 'disabled' : '' ?>">
                      ...
                      <? if ($cnt > 4 && $ch == count($arItem["VALUES"])):?>
...
<?php if (count($arResult['PROFILE_COMPANIES']) > 0 && empty($arParams['HIDE_COPMANY'])) { ?>
  <p class="personal-form__select-title">Выберите организацию</p>
  <form method="post" class="personal-form">
    <select class="personal-form__select" name="PROFILE_COMPANY" onchange="$(this).parents('form').submit()">
      <? foreach ($arResult['PROFILE_COMPANIES'] as $profile) { ?>
...
<script type="text/javascript">
  ...
  $('.bx_filter_parameters_open_all_open').click(function () {
    $(this).parent().find('.have_more').show();
    $(this).parent().find('.bx_filter_parameters_open_all_close').show();
    $(this).hide();
  });


  $('.bx_filter_parameters_open_all_close').click(function () {
    $(this).parent().find('.have_more').hide();
    $(this).parent().find('.bx_filter_parameters_open_all_open').show();
    $(this).hide();
  });
</script>

<style>
  .bx_filter_parameters_box_checkbox.form-check.disabled {
    display: none;
  }
</style>