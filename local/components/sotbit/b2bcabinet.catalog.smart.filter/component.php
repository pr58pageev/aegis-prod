...
if($this->StartResultCache(false, 'v9'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(isset($arParams["SHOW_SECTIONS"]) && $arParams["SHOW_SECTIONS"]=="Y")
    {
        $this->getItemSections();
        $arResult["ITEMS"] = $arResult["ITEMS"] + $this->getResultItems();
    }else
	...
	if(!empty($arResult["ITEMS"]))
	{
	    ...
	    if ($this->facet->isValid())
		{
			...
			$this->facet->setSectionsId($this->SECTIONS_ID);
            $this->facet->setElementsId($this->ELEMENTS_ID);
		...
		else
		{
			...
			if (!empty($this->arResult["PROPERTY_ID_LIST"]))
			{
				...
				while($arElement = $rsElements->Fetch())
                {
                    ...
                    if(isset($arResult["ITEMS"]["SECTION_ID"]))
                    {
                        $dbSect = CIBlockElement::GetElementGroups($arElement["IBLOCK_ELEMENT_ID"], true);
                        while($arSect = $dbSect->Fetch())
                            $arElements[$arElement["IBLOCK_ELEMENT_ID"]]["SECTION_ID"][] = $arSect["ID"];
                    }
	...
	if($this->SECTIONS_ID && $arResult["ITEMS"]["SECTION_ID"])
    {
        ...
        $rsSections = CIBlockSection::GetList(array("sort" => "asc"), $arSectionsFilter, false, array(
            "ID",
            "NAME",
        ));
...
if(isset($arResult["SECTIONS"])) $this->getItemSections();
	...
	foreach($arItem["VALUES"] as $key => $ar)
	{
		...
		)
		{
			...
			elseif($_CHECK[$ar["CONTROL_NAME"]] == $ar["HTML_VALUE"])
			{
				...
				if($PID == "SECTION_ID")
                {
                    $allCHECKED[$PID][$key] = true;
                    $arSectFilter[] = $key;
                }

				else
                    $allCHECKED[$PID][$ar["VALUE"]] = true;
				if ($arResult["FACET_FILTER"] && $PID != "SECTION_ID")
				{
...
if ($_CHECK)
{
	...
	if ($arResult["FACET_FILTER"])
	{
		if (!$this->facet->isEmptyWhere() || isset($arSectFilter))
		{
			foreach ($arResult["ITEMS"] as $PID => &$arItem)
			{
				if (/*$arItem["PROPERTY_TYPE"] != "SECTION_ID" && */$arItem["PROPERTY_TYPE"] != "N" && !isset($arItem["PRICE"]))
				{
            ...
            if(isset($arSectFilter))
                $this->sectionsId = $arResult["FACET_FILTER"]["SECTION_ID"] = $arSectFilter;
	...
	else
	{
		...
		foreach ($arResult["ITEMS"] as $PID => &$arItem)
		{
			if($arItem["PROPERTY_TYPE"]=="SECTION_ID")
            {
                $checked = $allCHECKED;
                unset($checked[$PID]);
                ...
                foreach ($arItem["VALUES"] as $key => &$arValue)
                {
                    ...
                    if (isset($index[$PID][$key]))
                    {//(array("ID"=>$PID, "KEY"=>$key));
                        //Check if there are any combinations exists
                        foreach ($index[$PID][$key] as $id => $combination)
                        {
		    ...
		    elseif ($arItem["PROPERTY_TYPE"] != "N" && !isset($arItem["PRICE"]))
			{
...
foreach($arResult["ITEMS"] as $PID => $arItem)
{
	...
	elseif($arItem["PROPERTY_TYPE"] == "SECTION_ID")
    {
        if(isset($arItem["VALUES"]) && !empty($arItem["VALUES"]))
        {
            //if(${$FILTER_NAME}[])
            foreach($arItem["VALUES"] as $id => $sect)
            {
                if($sect["CHECKED"])
                {
                    ${$FILTER_NAME}["SECTION_ID"][] = $id;
                }
            }
...
if(isset($arResult["ITEMS"]["SECTION_ID"]) && $arResult["FACET_FILTER"] && ${$FILTER_NAME} && isset($arResult["ITEMS"]["SECTION_ID"]))
{
    ...
    while ($el = $rs->GetNext())
    {
        if(isset($arResult["ITEMS"]["SECTION_ID"]["VALUES"][$el['IBLOCK_SECTION_ID']]))
        {
            $arResult["ITEMS"]["SECTION_ID"]["VALUES"][$el['IBLOCK_SECTION_ID']]["ELEMENT_COUNT"] = $el['CNT'];
            $arResult["ITEMS"]["SECTION_ID"]["VALUES"][$el['IBLOCK_SECTION_ID']]["DISABLED"] = false;
            if(!$el['CNT'])
                $arResult["ITEMS"]["SECTION_ID"]["VALUES"][$el['IBLOCK_SECTION_ID']]["DISABLED"] = true;

        }/*else{
            $arResult["ITEMS"]["SECTION_ID"]["VALUES"][$el['IBLOCK_SECTION_ID']]["ELEMENT_COUNT"] = 0;
            $arResult["ITEMS"]["SECTION_ID"]["VALUES"][$el['IBLOCK_SECTION_ID']]["DISABLED"] = true;
        }*/
    }
}
...
if(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y")
{
    ...
    if($this->SECTIONS_ID && (!isset($arFilter["SECTION_ID"]) || !is_array($arFilter["SECTION_ID"])))
        $arFilter["SECTION_ID"] = $this->SECTIONS_ID;
    if($this->ELEMENTS_ID)
        $arFilter["ID"] = $this->ELEMENTS_ID;
