...
if ($this->StartResultCache(false, array('v10', $preFilter, ($arParams["CACHE_GROUPS"] ? $USER->GetGroups() : false)))) {
    ...
    if (!empty($arResult["ITEMS"])) {
        if ($this->facet->isValid()) {
            ...
            $arElementFilter = array_merge($arElementFilter, ${$arParams['FILTER_NAME']});
            $arElements = array();
            ...
            //Проверка на получение Минимального и максимального
            //pre($arResult["ITEMS"][$NAME]);
...
elseif ($arParams["SAVE_IN_SESSION"] && isset($_SESSION[$FILTER_NAME][$this->SECTION_ID]))
    ...
    $_CHECK = array();
//pre($_REQUEST);pre($_GET);pre($_POST);die;
...
//pre($_CHECK);die;
    ...
    if (in_array($arItem['CODE'], $arResult['SLIDER_PROPS'])) {

        $sliderCode = $arParams['FILTER_NAME'] . '_' . $arItem['ID'];
        ...
        foreach ($arItem["VALUES"] as $key => $value) {
            ...
            $changeKoeff = 1;
            // устанавливаем коэффициент конвертации см в м, если это свойство Длина
            if ($arItem['CODE'] == 'DLINA' && strpos($myValue, 'см') !== false)
                $changeKoeff = 0.01;


            if (strpos($myValue, $arDivs) !== false) {
                foreach ($arDivs as $div) {
                    $tempVal = explode($div, $myValue);

                    if (count($tempVal) > 1) {
                        if ($div == '*') {
                            $totalSum = 0;
                            foreach ($tempVal as $k => $tVal) {
                                $tVal = preg_replace("/[^,.0-9]/", '', $tVal);
                                if ($totalSum == 0)
                                    $totalSum = $tVal;
                                else
                                    $totalSum *= $tVal;
                            ...
                            $totalSum = preg_replace("/[^,.0-9]/", '', $tempVal[0]);
                        }
                        $myValue = $totalSum;
                    }
                }
            ...
            // Конвертируем по коэффициенту
            $myValue = preg_replace("/[^,.0-9]/", '', $myValue) * $changeKoeff;


            if (empty($minMy) && $myValue > 0) {
        ...
        $arResult["ITEMS"][$PID]["VALUES"]["MAX"]['VALUE'] = $maxMy;
        $arResult["ITEMS"][$PID]["VALUES"]["MIN"]['VALUE'] = $minMy;
        $arResult["ITEMS"][$PID]["VALUES"]["MIN"]["HTML_NAME"] = $minHtml;
        $arResult["ITEMS"][$PID]["VALUES"]["MAX"]["HTML_NAME"] = $maxHtml;

// тут посчитать минимальные и макс значения для этого свойства
        $arResult["ITEMS"][$PID]["VALUES"]["MIN"]["CONTROL_NAME"] = $sliderCode . '_MIN';
        $arResult["ITEMS"][$PID]["VALUES"]["MIN"]["CONTROL_ID"] = $sliderCode . '_MIN';
        $arResult["ITEMS"][$PID]["VALUES"]["MAX"]["CONTROL_NAME"] = $sliderCode  . '_MAX';
        $arResult["ITEMS"][$PID]["VALUES"]["MAX"]["CONTROL_ID"] = $sliderCode  . '_MAX';

        $arItem = $arResult["ITEMS"][$PID];


        if (isset($_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]]) && $_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]]=='')
            unset($_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]]);


        if (isset($_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]]) && $_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]]=='')
            unset($_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]]);


            if (isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y") {
            foreach ($_CHECK as $check_key => $check_val) {
                if ($check_key != $arItem["VALUES"]["MIN"]["CONTROL_ID"] && $check_key != $arItem["VALUES"]["MAX"]["CONTROL_ID"])
                    if (strpos($check_key, $sliderCode) !== false)
                        unset($_CHECK[$check_key]);
            }
        } else {

            $HtmlValueMin = 100000000;
            $HtmlValueMax = 0;
            foreach ($arItem["VALUES"] as $key => $value) {

                if (isset($_CHECK[$value['CONTROL_NAME']])) {
                    $htmlValueNum = preg_replace("/[^,.0-9]/", '', $value['VALUE']);


                    if ($HtmlValueMin > $htmlValueNum)
                        $HtmlValueMin = $htmlValueNum;

                    if ($HtmlValueMax < $htmlValueNum)
                        $HtmlValueMax = $htmlValueNum;

                }
            }

            if ($HtmlValueMax !== 0)
                $arResult["ITEMS"][$PID]["VALUES"]["MAX"]['HTML_VALUE'] = $HtmlValueMax;

            if ($HtmlValueMin !== 100000000)
                $arResult["ITEMS"][$PID]["VALUES"]["MIN"]['HTML_VALUE'] = $HtmlValueMin;
        ...
        if ((isset($_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]]) && $_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]]!=='')
            ||
            (isset($_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]]) && $_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]]!=='')) {


            // 0) если не установлен MIN или MAX, то установить его минимальное(максимальное) значчение

            if (!isset($_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]]))
                $_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]] = $minMy;


            if (!isset($_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]]))
                $_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]] = $maxMy;


            $arResult["ITEMS"][$PID]["VALUES"]["MAX"]['HTML_VALUE'] = $_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]];
            $arResult["ITEMS"][$PID]["VALUES"]["MIN"]['HTML_VALUE'] = $_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]];

            // 1) пробегаемся по $arItem["VALUES"]
...
//pre($value);die;
                if ($myValue >= $_CHECK[$arItem["VALUES"]["MIN"]["CONTROL_ID"]] && $myValue <= $_CHECK[$arItem["VALUES"]["MAX"]["CONTROL_ID"]])
                    $_CHECK[$value['CONTROL_NAME']] = 'Y';
            }
            ///pre($_CHECK);die;
            // 2) приводим его значение к числу
            // 3) проверяем входимость в интервал
            // 4) дописываем в $_CHECKED их CONTROL_NAME
        }
...
if (!empty($_REQUEST['arrFilter_494_MAX']) || !empty($_REQUEST['arrFilter_494_MIN'])) {


    if (!empty($_REQUEST['arrFilter_494_MAX'])) {
        $max = $_REQUEST['arrFilter_494_MAX'];
    }

    if (!empty($_REQUEST['arrFilter_494_MIN'])) {
        $min = $_REQUEST['arrFilter_494_MIN'];
    }


    foreach ($arResult["ITEMS"] as $PID => $arItem) {
        if ($arItem['CODE'] == 'KOL_VO_LISTOV') {
    ...
    if ($arItem['CODE'] == 'KOL_VO_LISTOV') {
        ...
        foreach ($arItem["VALUES"] as $key => $value) {
            ...
            if ($myValue >= $min && $myValue <= $max) {
                ...
                $massiv[] = $value['CONTROL_NAME'];

                $arResult["ITEMS"][$PID]['VALUES'][$key]['CHECKED'] = 'Y';
