...
$promoIds = [];
$rs = CIBlockElement::GetList([],['ACTIVE' => 'Y','IBLOCK_ID' => 10,'!PROPERTY_LINK_PRODUCTS' => false],false,
    false,['PROPERTY_LINK_PRODUCTS']);
while($el = $rs->Fetch())
{
...
foreach ($this->basketItems as $key => $basketItem) {
    if ($basketItem['SET_PARENT_ID']) {
        unset($this->basketItems[$key]);
    }
}
...
{
    $ids[] = $row['PRODUCT_ID'];
    if($row['LABEL_ARRAY_VALUE'])
    {
        $codes = array_merge($codes,array_keys($row['LABEL_ARRAY_VALUE']));
    }
}
...
while ($ob = $res->fetch()) {
    ...
    $arResult['NAMES_PRODUCT_OFFERS'][$ob['NAME']] = $ob['PREVIEW_TEXT'];
...
foreach ($this->basketItems as $row)
{
    if($images[$row['PRODUCT_ID']]){
        $row = $Basket->changeImages($row,$images[$row['PRODUCT_ID']]);
    }

  

    $res = CIBlockElement::GetProperty($iblock_id, $row['PRODUCT_ID'], array("sort" => "asc"), Array("CODE"=>"POLNOE_NAIMENOVANIE"));
     while ($ob = $res->GetNext()) {
		...
		/*
		$koeff = 0;
		foreach ($row['SKU_DATA'] as $key => $value) {
					foreach ($value as $offer) {
					if ($offer['PROPERTIES']['CML2_MEASURE_UNIT_KOEF']['VALUE'] > 1)
					$koeff = $offer['PROPERTIES']['CML2_MEASURE_UNIT_KOEF']['VALUE'];
					}
		}
		*/
		 ...
		 $arInfo = CCatalogSKU::GetInfoByProductIBlock($iblock_id);
		  ...
		  $arOffersId = array();
		  $arSelect = array('*','PROPERTY_MAIN_MEASURE_UNIT','PROPERTY_CML2_MEASURE_UNIT_KOEF');
		
		  $rsOffers = CIBlockElement::GetList(array(),array(
		    'IBLOCK_ID' => $arInfo['IBLOCK_ID'],
		     'ID' => $row['PRODUCT_ID']
		 ), false, Array("nPageSize"=>1), $arSelect); 
		    while ($arOffer = $rsOffers->fetch()) {
		   
		     if($arOffer['PROPERTY_MAIN_MEASURE_UNIT_VALUE']=='Нет')
		     {
		     	$row['KOEF'] = $arOffer['PROPERTY_CML2_MEASURE_UNIT_KOEF_VALUE'];

		     }
		 } 

		 
		 $row['QUANT_KOEF'] = $row['QUANTITY'] * $row['KOEF'];

		 foreach ($row['PROPS'] as $key => $value) {
		 	if($value['CODE']=='CML2_MEASURE_UNIT'){
		 			$row['MEASURE_TEXT'] = $row['PROPS'][$key]['VALUE'];
		 	}
		 }
		

		if($row['KOEF']>1){
			$row['MESSAGE_TXT']="Кол-во в упаковке: ";
		}else{
			unset($row['QUANT_KOEF']);
			unset($row['KOEF']);
		}
		if(!empty($arResult['NAMES_PRODUCT_OFFERS'][$row['MEASURE_TEXT']])):
	$row['MEASURE_TEXT'] = $arResult['NAMES_PRODUCT_OFFERS'][$row['MEASURE_TEXT']];
endif;
	if(!empty($arResult['NAMES_PRODUCT_OFFERS'][$row['PROPS'][0]['VALUE']])):
	$row['PROPS'][0]['VALUE'] = $arResult['NAMES_PRODUCT_OFFERS'][$row['PROPS'][0]['VALUE']];
	endif;
	if(!empty($arResult['NAMES_PRODUCT_OFFERS'][$row['PROPS_ALL']['CML2_MEASURE_UNIT']['VALUE']])):
	$row['PROPS_ALL']['CML2_MEASURE_UNIT']['VALUE'] = $arResult['NAMES_PRODUCT_OFFERS'][$row['PROPS_ALL']['CML2_MEASURE_UNIT']['VALUE']];
	endif;
		...
		foreach ($row['SKU_DATA'] as $key => $value) {
			...
			$row['SKU_DATA'][$key]['VALUES'][$key2]['NAME2']	= $arResult['NAMES_PRODUCT_OFFERS'][$value2['NAME']];
		...
		foreach ($row['SKU_DATA'] as $key => $value) {
			foreach ($value['VALUES'] as $key2 => $value2) {
				...
				if(empty($row['SKU_DATA'][$key]['VALUES'][$key2]['NAME2'])):
					$row['SKU_DATA'][$key]['VALUES'][$key2]['NAME2'] = $row['SKU_DATA'][$key]['VALUES'][$key2]['NAME'];
				endif;
	...
	$rowData = array(
		...
		"KOEF"=>$row['KOEF'],
		"MESSAGE_MEASURE" => $row['MESSAGE_MEASURE'],
		"QUANT_KOEF"=> $row['QUANT_KOEF'],
	...
	if (!empty($row['SKU_DATA']))
	{
		...
		{
			if(empty($skuBlock['NAME2'])):
				$skuBlock['NAME2'] = $skuBlock['NAME'];
			endif;
    ...
    if (!empty($row['LABEL_ARRAY_VALUE']))
    {
        ...
        $lcodes = array_keys($row['LABEL_ARRAY_VALUE']);
        $hints = [];

        $rs = \Bitrix\Iblock\PropertyTable::getList(['filter' => ['CODE' => $lcodes],'select' => ['CODE','HINT']]);
        while($prop = $rs->fetch())
        {
            $hints[$prop['CODE']] = $prop['HINT'];
        }
        foreach ($row['LABEL_ARRAY_VALUE'] as $code => $value)
        {
            $style = 'background:#00B02A';

            if($hints[$code])
            {
                $style = 'background:'.$hints[$code];
            }

            $labels[] = array(
                'NAME' => ($props[$code])?$props[$code]:$value,
                'HIDE_MOBILE' => !isset
                ($this->arParams['LABEL_PROP_MOBILE'][$code]),
                'CLASS' => 'sticker_product__hit',
                'STYLE' => $style
            );
        }
    ...
    if($promoIds && in_array($rowData['PRODUCT_ID'],$promoIds))
    {
        $rowData['LABEL_VALUES'][] = $labels[] = array(
            'NAME' => Loc::getMessage('SBB_PROMOTION'),
            'HIDE_MOBILE' => 1,
            'CLASS' => 'sticker_product__sale'
        );
    }
...
$minSummOrder = $this->arParams['MIN_SUMM'];
	...
	$totalData['MIN_SUMM'] = $minSummMustache;
