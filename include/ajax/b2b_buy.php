...
else if(!\Bitrix\Main\Loader::includeModule('sotbit.b2bcabinet'))
{
    $errors[] = ['STATUS' => 'ERROR','MESSAGE' => 'Module catalog not installed'];
}
else
{   
    ...
    if(!empty($_SESSION['BLANK_IDS']))
        $arProduct = $_SESSION['BLANK_IDS']['ELEMENTS'];
    ...
    if(is_array($arProduct))
    {
        foreach ($arProduct as $key => $item)
        {
            ...
            foreach ($item['PROPS'] as $key2 => $value2) {
                if($value2['CODE']=='CML2_ARTICLE' ){
                   // unset($item['PROPS'][$key2]);
                }
            }
            ...
            if(!Add2BasketByProductID($key, $item['QNT'], $fields2, $item['PROPS']))
            {
